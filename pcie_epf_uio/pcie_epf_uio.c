...
/*
 * Simple driver to test endpoint functionality with UIO
...
static const char *BAR_NAMES[PCI_STD_NUM_BARS] = {
	"BAR0",
	"BAR1",
	"BAR2",
	"BAR3",
...
static size_t bar_size[] = { 4096, 4096, 4096, 16384, 131072, 1048576 };
...
static void pci_epf_simple_unbind(struct pci_epf *epf)
{
	...
	uio_unregister_device(epf_simple->uio_info);
}
...
static int pci_epf_simple_alloc_space(struct pci_epf *epf)
{
	...
	struct uio_info *info = epf_simple->uio_info;
	...
	for (bar = 0; bar < PCI_STD_NUM_BARS; bar += add) {
		...
		/* Set UIO Memory info for BARs */
		if (bar < MAX_UIO_MAPS) {
			dev_info(dev, "Setting UIO Map info for bar : %d\n", bar);
			info->mem[bar].name = BAR_NAMES[bar];
			info->mem[bar].size = bar_size[bar];
			info->mem[bar].memtype = UIO_MEM_PHYS;
			info->mem[bar].addr = epf->bar[bar].phys_addr;
			info->mem[bar].internal_addr = epf->bar[bar].addr;
		}
	}
...
static int pci_epf_simple_bind(struct pci_epf *epf)
{
	...
	dev_info(&epf->dev, "Binding PCI EPF\n");
	...
	WRITE_ONCE(((struct pci_epf_simple_reg *)epf_simple->reg[simple_reg_bar])->magic, 
		     0xbeefbeef);
	info->name = "PCIe endpoint fun";
	info->version = "0.0.1";
	info->irq = UIO_IRQ_NONE;

	dev_info(&epf->dev, "Registering UIO device\n");
	dev_info(&epf->dev, "First memory map size : %llu\n", info->mem[0].size);
	dev_info(&epf->dev, "First memory map phys_addr : 0x%p\n", (void *)info->mem[0].addr);
	dev_info(&epf->dev, "First memory map addr : 0x%p\n", info->mem[0].internal_addr);

	if (uio_register_device(&epf->dev, epf_simple->uio_info)) {
		dev_err(&epf->dev, "Cannot register as a UIO device");
		return -ENODEV;
	}
...
static int pci_epf_simple_probe(struct pci_epf *epf)
{
	...
	epf_simple->uio_info = devm_kzalloc(dev, sizeof(struct uio_info), GFP_KERNEL);
	if (!epf_simple->uio_info)
		return -ENOMEM;
